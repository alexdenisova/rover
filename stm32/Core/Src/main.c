...
static void UART_Receive_ESP32() {
  HAL_UARTEx_ReceiveToIdle_IT(&huart1, uart_rx, UART_RX_BUFFER_SIZE);
}
/* USER CODE END 0 */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOC, Recording_LED_Pin|Rover_Open_Pin|Rover_Close_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, SD_CS_Pin|Rover_Forward_Pin|Rover_Backward_Pin|Rover_Right_Pin
  ...
  GPIO_InitStruct.Pin = Rover_Forward_Pin|Rover_Backward_Pin|Rover_Right_Pin|Rover_Left_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_PULLDOWN;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  ...
  if (GPIO_Pin == GPIO_PIN_13) {
    ...
    if ((uxBits & BIT_0) == 0) { // If Event flag not set
      UART_Log("Recording\n");
      HAL_GPIO_WritePin(Recording_LED_GPIO_Port, Recording_LED_Pin, GPIO_PIN_SET); // Turn on recording LED
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
  ...
  for (int i = 0; i < ADC_BUFFER_SIZE; i++) {
    ...
    if (samples_taken == NUMBER_OF_SAMPLES) {
      ...
      HAL_GPIO_WritePin(Recording_LED_GPIO_Port, Recording_LED_Pin, GPIO_PIN_RESET); // Turn off recording LED
      samples_taken = 0;
    }
  }
}
